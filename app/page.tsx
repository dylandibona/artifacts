...
export default function Home() {
  ...
  const [artistModel, setArtistModel] = useState("flux"); // Default to 'flux'
      ...
      <div className="max-w-2xl w-full">
        ...
        <div className="bg-[#e07a5f]/10 p-6 md:p-8 rounded-xl border-2 border-[#3d405b] shadow-[8px_8px_0px_0px_rgba(61,64,91,1)]">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Choose Your Artist */}
            <div>
              <label className="block font-semibold uppercase mb-2 text-sm tracking-wide" style={{ fontFamily: "var(--font-body)" }}>Choose Your Artist</label>
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  onClick={() => setArtistModel("flux")}
                  className={`p-4 rounded-lg border-2 transition-all text-left ${
                    artistModel === "flux"
                      ? "border-[#cc5500] bg-[#cc5500]/10 shadow-[2px_2px_0px_0px_rgba(204,85,0,1)]"
                      : "border-[#3d405b] bg-[#f4f1de] hover:bg-[#e07a5f]/10"
                  }`}
                >
                  <div className="text-2xl mb-1">📸</div>
                  <div className="font-bold text-sm uppercase mb-1" style={{ fontFamily: "var(--font-body)" }}>The Photographer</div>
                  <div className="text-xs opacity-70" style={{ fontFamily: "var(--font-mono)" }}>Hyper-real, gritty, sharp text.</div>
                </button>
                <button
                  type="button"
                  onClick={() => setArtistModel("dalle")}
                  className={`p-4 rounded-lg border-2 transition-all text-left ${
                    artistModel === "dalle"
                      ? "border-[#cc5500] bg-[#cc5500]/10 shadow-[2px_2px_0px_0px_rgba(204,85,0,1)]"
                      : "border-[#3d405b] bg-[#f4f1de] hover:bg-[#e07a5f]/10"
                ...
                >
                  <div className="text-2xl mb-1">🎨</div>
                  <div className="font-bold text-sm uppercase mb-1" style={{ fontFamily: "var(--font-body)" }}>The Illustrator</div>
                  <div className="text-xs opacity-70" style={{ fontFamily: "var(--font-mono)" }}>Vibrant, creative, colorful.</div>
                </button>
              </div>
            </div>
        ...
        {generatedImage && (
          <div className="mt-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <p className="text-center mt-6 text-sm opacity-50" style={{ fontFamily: "var(--font-mono)", fontWeight: 400 }}>
              *Images generated by {artistModel === "flux" ? "Flux 1.1 Pro" : "DALL-E 3"}
            </p>
          </div>
        )}
      </div>
